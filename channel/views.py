...
def channel_data(request):
    ...
    # ── Aggregate KPIs ───────────────────────────────────────
    agg_fields = {
        'fca_target':         Sum('fca_target'),
        'fca_ach':            Sum('fca_ach'),
        'g4_target':          Sum('g4_target'),
        'g4_ach':             Sum('g4_ach'),
        'mnp_target':         Sum('mnp_target'),
        'mnp_ach':            Sum('mnp_ach'),
        'mbb_target':         Sum('mbb_target'),
        'mbb_ach':            Sum('mbb_ach'),
        'loading_target':     Sum('loading_target'),
        'loading_ach':        Sum('loading_ach'),
        'm0_revenue_target':  Sum('m0_revenue_target'),
        'm0_revenue_ach':     Sum('m0_revenue_ach'),
        'bundle_target':      Sum('bundle_target'),
        'bundle_ach':         Sum('bundle_ach'),
        'hvc_target':         Sum('hvc_target'),
        'hvc_ach':            Sum('hvc_ach'),
        'qos_target':         Sum('qos_target'),
        'qos_ach':            Sum('qos_ach'),
...
def build_growth_dict(qs_base, year_param, month_param):
    ...
    ytd_curr = agg_for(Q(date__year=ly,      date__month__lte=lm))
    ytd_prev = agg_for(Q(date__year=prev_y,  date__month__lte=lm))
    mtd_curr = agg_for(Q(date__year=ly,      date__month=lm))
    mtd_prev = agg_for(Q(date__year=prev_m_y,date__month=prev_m))
    yoy_curr = agg_for(Q(date__year=ly,      date__month=lm))
    yoy_prev = agg_for(Q(date__year=prev_y,  date__month=lm))
...
def build_monthly_trend(qs):
    ...
    metric_list = [
        'fca_ach', 'mnp_ach', 'mnp_target', 'mbb_ach',
        'loading_ach', 'loading_target',
